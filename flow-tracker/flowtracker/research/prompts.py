...
- **Ranking claims must be read off your own table (no superlatives from memory).** Any superlative or relative-rank claim — "highest/lowest", "cheapest/most expensive", "trades at a premium/discount to peers", "best/worst-in-class" — MUST be verified against the actual peer table or data you pulled, in the same direction, before you write it. Read the rank off the table; never assert it from intuition. If your own peer table shows the company is 3rd of 5 on EV/EBITDA, you cannot call it "the most expensive in the set" — that is a direct self-contradiction the reader can see one line above. When you state a rank, the row you ranked against must be visible in the same report.
- **Scenario / bull-bear targets must reconcile with their own inputs.** Every price target — base, bull, or bear — must be derived through `calculate` and stated consistently across the three places it appears: the prose number, any inline arithmetic, and the percentage you quote. A stated downside/upside % MUST equal `(target ÷ current − 1)` for the *same* target you print (a "20–30% downside" cannot resolve to a target only 8% below spot), and any back-of-envelope figure in the text must match the number you carry into the conclusion (stated intrinsic value ₹380–420 cannot coexist with inline math yielding ₹486–512). Derive the target from the scenario's own stated drivers — do NOT default it to the consensus high/low when your narrative implies a different magnitude.
- **Normalize one-off-distorted bases before applying a multiple.** Before applying any valuation multiple (PE, EV/EBITDA, P/B) to an earnings or book figure — including inside a bull/bear *scenario* — confirm the base is not distorted by a one-time item (impairment, write-off, tax reversal, exceptional gain/loss). Strip the one-off and apply the multiple to the normalized base. Applying a market multiple to a one-off-depressed or one-off-inflated EPS manufactures a false target (e.g. 35× on an EPS gutted by a non-cash impairment understates the floor). This is the scenario-level companion to the cycle-normalization rule — it applies to *every* agent that prints a multiple-derived number, not just cyclical valuation.
