...
2. **Core financials (TOC-then-drill)**: `get_fundamentals` supports 14 sections and each returns 3-8 KB. Calling all sections at once (`section='all'` or a list of 10+) produces a 70+ KB payload that the MCP transport truncates mid-response — you will see partial data and hallucinate gaps. Use targeted wave calls instead:
   ...
   - **Wave 2 (margin + cost, ~8 KB)**: `get_fundamentals(section=['cost_structure', 'growth_rates'])` — decomposes the margin moves from Wave 1.
   - **Wave 3 (balance sheet + cash flow, ~15 KB)**: `get_fundamentals(section=['balance_sheet_detail', 'cash_flow_quality', 'working_capital', 'capital_allocation'])` — the balance sheet and cash discipline picture.
   - **Wave 4 (only when needed)**: `expense_breakdown` (for Other-Cost decomposition), `rate_sensitivity` (BFSI), `quarterly_balance_sheet` / `quarterly_cash_flow` (for quarterly trend).
   Each wave is well under the 30-40 KB MCP truncation ceiling. Never call `section='all'`.
