...
| **Commercial / Office REIT** (EMBASSY, MINDSPACE, BIRET) | Leased area × rent psf × occupancy | rent roll + occupancy | NOI per sqft, WALE, cap rate ~7.5-8.5% prime, up to ~9% Tier-2 |
...
| **Specialty — data-centre** (emerging) | MW-leased × rent per MW × uptime | power-capacity × tenancy | revenue per MW, cap rate ~7-8.5% (institutional demand has compressed yields well below the old 9-11% assumption; 15-yr NNN hyperscaler development yields ~7.5-8.5%) |
...
- **Distribution / pre-launch bookings** — the ability to convert a large share of a tower into bookings within days of a RERA-registered launch (via channel-partner depth and an existing customer waitlist) is a brand-plus-channel moat that compresses the capital cycle. Note: advertising, marketing, booking, or selling *before* RERA project registration is prohibited under Section 3 of the RERA Act, 2016 — "soft-launch" selling pre-registration is illegal, not a moat. The legitimate edge is launch-day absorption velocity post-registration.
...
- **Leverage — Net Debt / Equity rising (or Net Debt / OCF deteriorating)** in a flat or decelerating presales environment is the classic cycle-trap setup. Avoid Net Debt / EBITDA as the leverage gauge for residential developers: Ind AS 115 defers EBITDA to handover while debt funds *current* construction, so the ratio is structurally mismatched and overstates leverage in buildout years. Use Net Debt / Equity (>1.0x elevated) and Net Debt / collections-OCF instead.
