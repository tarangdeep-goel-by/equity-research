...
**Why 2.5-3.5 is the fair PEG band for Indian tier-1 IT (not textbook 1.0-1.5):** four balance-sheet attributes compound earnings quality above raw growth. (a) **FCF conversion 80-100% of PAT** — earnings translate cleanly into distributable cash, unlike capex-heavy sectors. (b) **High capital-return payout** — tier-1 capital-allocation policies mandate returning ~70-100% of FCF/PAT to shareholders (e.g. TCS targets 80-100% of FCF; Infosys ~85% over a 5-year cycle) via dividends + buybacks, so ongoing cash yield is a material total-return component. (c) **Net-cash balance sheet** — no solvency discount. (d) **<2% maintenance capex** — incremental growth flows fully to FCF. Strip any of these and the band compresses toward standard 1.5-2.5 PEG territory.
...
- **Simple PE on peak-margin year** — even at the FY22 post-COVID peak only TCS reached ~25% operating margin; Infosys was ~23% and HCLTech/Wipro lower (high-teens to ~19%), and the set compressed further to 22-24% (top names) / mid-to-high-teens (others) by FY25. Don't assume a uniform 26-27% tier-1 peak — comparing peer PE on an over-stated peak-margin EPS over-states value.
