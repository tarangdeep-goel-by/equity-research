...
- **Logistics / dark-store density** — for quick-commerce, 10-30 min SLAs require high dark-store density per city; category leaders operate 1,500+ stores nationally (hundreds per top metro), not the dozens an earlier-stage model implies. Once built, the fixed-cost amortisation is a moat; before it, it is a cash-burn engine.
...
- **Dark-store capex economics (quick-commerce)** — per-store capex ₹45-60L (fit-out, cold chain, racking, WMS integration; UBS 2026 benchmark — automated stores run higher, ₹50-75L), with payback measured in months at mature utilisation but stretching to 18+ months in weaker tier-2 catchments. Breakeven throughput is ~800-1,500 orders/day/store depending on city tier (a Mumbai/metro store needs the higher end to be profitable; a tier-2 store at ~400 orders/day may never break even); a single dark-store can process up to ~1,400 orders/day at scale. Geographic-density is load-bearing: leaders now run 1,500+ stores nationally (hundreds per top metro to sustain 10-minute SLAs), and metros sustain multiple stores/sq-km at full density — sub-scale store-count per city means diseconomies of fulfilment (longer rider routes, higher per-order logistics cost). Dark-store network economics therefore flip regime at city-level saturation — model each metro's saturation cohort separately, not aggregate store count.
...
- **FDI Press Note 2 structural non-compliance (foreign-funded marketplaces)** — a foreign-funded e-commerce *marketplace* entity cannot own/control inventory (no 1P) and cannot have >25% of any single vendor's sales routed through it/its group, nor hold equity in a vendor selling on the platform. Audit for: a large reported-1P book sitting inside the foreign-funded entity (vs a separate domestically-controlled inventory/wholesale arm); "preferred/alpha seller" structures where group-affiliated sellers dominate GMV (the 25%-vendor and equity-in-vendor traps); and any CCI/ED scrutiny of these structures. Press Note 2 breach is an existential, not cosmetic, risk — it can force a wholesale restructuring of the operating model. This does not bite quick-commerce/food-delivery inventory that sits outside the "marketplace" definition or in compliant entities — so first classify the model and the holding entity, then assess.
