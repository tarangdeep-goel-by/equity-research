...
- ETERNAL: food delivery + Hyperpure (B2B) + Blinkit (quick commerce) + District (going-out: dining-out, movie/event/sports ticketing) — all four need separate treatment once individually material. Note: Blinkit is the quick-commerce arm; District is the going-out/ticketing business (built on the acquired Paytm entertainment/ticketing assets) — do not conflate them.
- PAYTM: UPI / payments + lending + commerce — each vertical separate.

## Projections Tool Caveat — Asset-Light (new)

`get_projections(section='income_statement')` applies a default D&A assumption suitable for manufacturing companies (~5% of revenue). For asset-light pure platforms (3P marketplaces, payments, insurtech distribution), that assumption is too high — override with an asset-turnover-based projection (0.5–1.5% of revenue). **Important exception — dark-store-heavy quick-commerce is NOT asset-light for D&A purposes:** under Ind AS 116, dark-store (and warehouse) leases are capitalised as right-of-use (ROU) assets, so rent moves out of operating cost and into ROU-asset depreciation (D&A) plus lease-liability interest. This makes D&A materially higher than the 0.5–1.5% pure-platform band for quick-commerce and other lease-intensive fulfilment models — do not apply the asset-light override there. If the projections tool already emits a `_projection_assumptions` meta field (see E12 upgrade), use the routed ratio; otherwise note the mis-applied assumption explicitly in your valuation and adjust downstream.
