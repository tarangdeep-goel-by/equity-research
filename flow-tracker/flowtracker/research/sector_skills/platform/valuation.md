...
**FDI guardrail on the 1P share:** under FDI Press Note 2 of 2018, a *foreign-funded e-commerce marketplace entity* cannot own/control inventory (no 1P) and cannot have >25% of a vendor's sales routed through it. So a large 1P book on a foreign-funded marketplace is not just a margin choice — it is a structural/regulatory flag (the 1P is typically held in a separate domestically-controlled entity, JV, or a wholesale-cash-and-carry arm). When you see a material reported-1P share, first confirm *which legal entity* holds the inventory and whether the structure is FDI-compliant; quick-commerce and food-delivery inventory models often sit outside the "marketplace" definition or use such structures. Do not normalise a 1P/3P blend without noting the entity/FDI structure behind the 1P revenue.

### 1P / Inventory-Led Platforms — Engage the Retail Balance Sheet
For inventory-led (1P) models (e.g. a BPC or fashion e-tailer that takes inventory onto its own books), the valuation must engage the working-capital cycle, not just GMV growth: report **inventory days, payable days, and cash-conversion cycle** from `get_fundamentals(section='working_capital')` (a deteriorating CCC funds growth with working capital and is a quality flag), and **AOV (average order value) + order-volume trend** from `get_company_context(section='concall_insights', sub_section='operational_metrics')`. GMV growth driven by AOV inflation vs order-volume growth are different-quality stories — name which. If AOV / order count are not disclosed, state the gap rather than narrate GMV alone.
...
At mature-scale the premium must also reconcile with a BFSI-style Gordon tie — but the justified multiple is bounded by the steady-state **cash flow per unit of GMV**, not by take-rate alone. Take-rate is gross revenue/GMV; the value accrues only on the FCF that revenue ultimately converts to, so a steady-state FCF (or owner-earnings) margin on revenue MUST sit in the numerator:
```
Justified EV/GMV ≈ take-rate × steady_state_FCF_margin × (1 + g_gmv) / (WACC − g_gmv)
```
where `g_gmv` is the sustainable-GMV-growth rate (not the trailing 2-year burst) and `steady_state_FCF_margin` is FCF as a % of platform revenue at maturity. The earlier `take-rate × (1+g)/(WACC−g)` form implicitly assumes a 100% FCF margin on revenue and massively overstates the justified multiple. **Worked example:** peer median take-rate 20%, steady-state FCF margin on revenue 30%, `g_gmv` 12%, WACC 14% → justified EV/GMV ≈ 0.20 × 0.30 × 1.12 / 0.02 ≈ 3.4×. Subject stock at 4.2× trades at a ~25% premium; decompose the spread into take-rate, FCF-margin, g_gmv and WACC deltas — note the denominator (WACC − g_gmv) is highly sensitive, so a 200 bps g_gmv spread that pulls it from 2% toward 0% dominates. If the take-rate + FCF-margin + g + WACC spreads do not arithmetically rebuild the premium, the residual is re-rating risk. Run the tie through `calculate` with named inputs `take_rate_pct`, `fcf_margin_pct`, `g_gmv_pct`, `wacc_pct`.
