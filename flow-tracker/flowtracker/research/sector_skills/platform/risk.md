...
- **Selective / repeated equity issuance pattern** — note that preferential allotments do NOT escape shareholder approval: under s.62(1)(c) of the Companies Act 2013, every preferential allotment requires a special resolution (75% of votes cast). The governance concern is therefore not "avoiding a vote" but *who* the shares go to and at what price — repeated preferential allotments or private placements to related/affiliated parties, pricing close to the ICDR floor, or a pattern of issuances that dilute minority holders while routing equity to insiders.
...
- **IRDAI** — for insurtech: the 2023 reform (IRDAI Payment of Commission Regulations + EoM Regulations, effective 1 Apr 2023) **removed the erstwhile product-wise individual commission caps** and replaced them with an overall entity-level **Expenses of Management (EoM) ceiling** — distribution economics now flex within the insurer's board-approved policy under that EoM cap rather than fixed per-product limits; product-approval regime still applies.
...
- **Data-localisation / cross-border-transfer** — note these are two *different* regimes. **RBI's 2018 payments-data mandate** is a genuine hard-localisation rule (payment-system data must be stored only in India), and non-compliance can shut down foreign-cloud payment dependencies. The **DPDP Act 2023 is NOT a data-localisation law** — it uses a "negative list" (Section 16): personal data may be transferred to any jurisdiction *except* those the Central Government specifically blacklists. Do not frame DPDP as mandating localisation; the localisation exposure for a platform comes from the RBI payments rule (and any sectoral RBI/SEBI directions), not from DPDP.
...
- **State-level gig-worker welfare cess (direct contribution-margin hit)** — distinct from classification risk, several states now levy a per-transaction welfare fee ON THE AGGREGATOR that flows straight to the welfare fund and cannot be fully passed through. Rajasthan (Platform-Based Gig Workers Act 2023, first-mover, rules still being framed) and Karnataka (Act 2025, welfare fee set at ~1-1.5% of each transaction, revised down from a proposed 5%) are live; other states are drafting similar laws. Model this as a **~1-2% direct hit to per-order contribution margin** in exposed states (not a generic regulatory note) — for a fulfilment-heavy platform already at 2-6% CMPO, a 1-2% cess can erase a third-to-half of unit contribution. Quantify GMV share in cess-active states and run the margin sensitivity through `calculate`.
