...
- **Top-2 CV OEMs + tractor leader** — M&HCV leader, multi-segment CV + tractor leader; tractor market is moderately concentrated — the M&M group (Mahindra + Swaraj) leads at ~40-43% with TAFE the clear #2, so the top-2 hold roughly 55-65% (not the 75-85% sometimes cited).
- **3W landscape (not a duopoly)** — overall 3W is led by Bajaj (~36%) with Piaggio, Mahindra and others behind; ICE 3W is a Bajaj + Piaggio contest while **electric** 3W is led by Mahindra. 3W is India's fastest-EV-adopting sub-segment (~57% EV penetration in FY25, well ahead of 2W/4W). EV-mandate tailwind (state-level ICE-3W registration restrictions in metros + fleet-operator TCO parity) is structurally re-rating the winners; last-mile-logistics demand tied to e-commerce penetration is the demand engine.
