...
**Non-Disposal Undertakings (NDUs) — shadow pledges.** Promoters sometimes secure lender debt against their shares via an NDU (a contractual commitment not to sell/transfer) rather than a formal pledge, which avoids the formal "pledge" disclosure trigger and can mask true liquidity distress. Scan `filings` for NDU language; where found, add the NDU-encumbered quantum to the formal pledge % to get the real encumbrance picture — an apparently zero-pledge promoter with a large NDU is not actually unencumbered.
...
### PLI / PM E-DRIVE Subsidy Reliance & Capex Execution
Government subsidy structures (Auto + Auto-component PLI ₹25,938 Cr, ACC battery PLI ₹18,100 Cr, and the PM E-DRIVE demand-incentive scheme — FAME-II expired March 2024) tie financial incentives directly to production-volume and local-value-addition (DVA) milestones. Stable promoter ownership correlates strongly with the ability to execute the multi-year capex required to meet these thresholds. Look for QIPs meant to fund PLI-linked capacity. Interrogate `get_company_context(section='concall_insights')` to connect ownership structure's capital-raising ability with management commentary on realizing PLI / PM E-DRIVE subsidy cash flows.
...
Any auto / auto-adjacent stock listed <730 days ago — OLAELEC (pure-play EV), ATHER (recently listed EV 2W), SWIGGY (peer-listed platform, same post-IPO SEBI (ICDR) Reg 16/17 cycle even though non-auto) — requires a populated lock-in calendar in Section 2. (Do NOT include legacy listings here: KIRLOSBROS listed 2006 and UNOMINDA, formerly Minda Industries, has traded since the early 2000s — neither is a recent IPO.) Without it, FII / VC exit narration during the first 540 days is incomplete and 30/90/180-day expiries get collapsed into a single "1-year lock-in" story:
...
| T+180d | Excess promoter holding (above MPC) — 6m, non-capex case | X% | locked / expired |
| T+365d | Excess promoter holding — 1yr, capex-funded fresh-issue case | X% | locked / expired |
| T+540d | Minimum promoter contribution (20% of post-issue) — 18m SEBI floor (3yr if capex-heavy) | X% | locked / expired |

**SEBI (ICDR) Amendment 2025 (notified 3 Mar 2025) — current lock-in tenors:** MPC (20% of post-issue capital) locks in for **18 months** (or **3 years** if the majority of fresh-issue proceeds fund capex); **excess** promoter holding above MPC locks in for **6 months** (or **1 year** in the capex case); non-promoter pre-issue capital locks in for **6 months**. Pre-2025 issues carried the longer 1-year / 3-year tenors, so check the RHP date before applying these. Dates sourced from the DRHP / RHP via `get_company_context(section='filings', query='lock-in|RHP|allotment')`. Do NOT fold 30-day / 90-day / 180-day expiries into a single "one-year lock-in" summary — these are distinct cliffs and the FII selloff is typically concentrated in the 30-60 day window AFTER each cliff (Tenet 19).
...
*Pattern applies to*: OLAELEC IPO quarter, ATHER (on listing), and any other genuinely recent auto IPO OFS event — verify via `corporate_actions` + `filings` for the RHP OFS quantum, then cross-check against `get_ownership(section='insider')` for the post-listing window to confirm no SAST events coincide.
