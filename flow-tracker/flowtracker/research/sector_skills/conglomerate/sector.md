...
- **Holding-company specialists / pure holdcos** — limited-to-no operating business at the parent, value entirely in listed-sub stakes; structurally the widest holdco discount cohort. Indian listed pure-holdcos (treasury + listed-stake only, no operating P&L) have historically traded at **40-65% discount to NAV**, materially wider than operating-plus-holdings structures (20-35%). The wider discount reflects no operating cash-flow optionality, tax friction on dividend upstreaming (a pure holdco that retains rather than re-distributes dividends loses the Section 80M deduction, which only applies to the extent dividends are passed through to its own shareholders — so the leakage is real for a hoarding holdco but avoidable for a pass-through one), and the absence of a standalone re-rating catalyst beyond NAV compression
...
- **SEBI special call-auction for investment holdcos (Jun 2024)** — SEBI introduced a once-a-year special call-auction mechanism (no price bands) specifically to improve price discovery for listed Investment Companies / Investment Holding Companies trading at deep NAV discounts; it is a structural, regulator-driven discount-compression catalyst distinct from corporate action. For a holdco-archetype name, assess eligibility and any historical price-discovery outcome under this mechanism
...
- **Regulatory tightening on related-party transactions** — SEBI's 2021-22 LODR amendments lowered material-RPT thresholds and expanded disclosure; the 2025 LODR Fifth Amendment then recalibrated to a *scale-based* materiality framework (Schedule XII, capped at ₹5,000 Cr) that *eases* the approval burden for the largest groups while preserving minority protection. Material RPTs need shareholder approval by **ordinary** resolution (not special). Group structures carrying high RPT are gradually re-pricing
