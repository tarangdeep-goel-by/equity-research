...
**"Others / Miscellaneous" breakout — analytically mandatory above 15% (note: not legally mandatory).** Be precise on the standard: Ind AS 108 only requires that reportable segments together cover **at least 75% of external revenue** (para 15), so an "Others" bucket of *up to ~25%* is fully compliant — there is no legal obligation to break it out. The 15% trigger here is an *analyst* threshold, not a reporting rule: an opaque 15%+ catch-all defeats SOTP and hides cross-subsidy, so for your own valuation you should still demand the decomposition. Pull it from `get_company_context(section='concall_insights', sub_section='management_commentary')` or the annual-report segment note in `get_company_context(section='filings', sub_section='notes_to_accounts')`; if it cannot be decomposed, name that as a segment-reporting *opacity* concern (a disclosure-quality flag, not a compliance breach) rather than letting it pass.
...
- **Section 186 (Companies Act) capital-mobility ceiling** — inter-corporate loans, investments, and guarantees are capped at the *higher* of 60% of (paid-up capital + free reserves + securities premium) or 100% of (free reserves + securities premium); exceeding it needs a **special resolution** (loans/guarantees to a wholly-owned subsidiary or JV are exempt). For a conglomerate funding many subs, check whether aggregate inter-corporate exposure is approaching this ceiling — a pending Sec 186 special resolution is a constraint on (and a signal about) how aggressively the parent is recycling capital into group entities
