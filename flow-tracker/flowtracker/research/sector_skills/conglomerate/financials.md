...
Under Ind-AS 28, investments in associates and JVs are equity-accounted based on **significant influence** or **joint control** — not a strict 20-50% ownership band. The ≥20% voting power is only a *rebuttable presumption* of significant influence (and <20% the rebuttable presumption against it); an entity can even hold >50% yet still equity-account if it has only joint control rather than control. Under the equity method only the investor's share of the JV/associate's net income appears on the parent's P&L as a single line. The JV's revenue, EBITDA, and margins are **excluded** from the parent's consolidated financials entirely.
