...
SOTP gives the bottom-up sub-of-parts value; reverse DCF on consolidated earnings is the top-down sanity check. When SOTP implies 25% upside but reverse DCF on consolidated PAT implies only 10% upside, the gap almost always means SOTP is over-counting subsidiary value that is not actually distributed to the parent in dividends. Reconcile before publishing. Realistic Indian nominal `g` for the reverse-DCF terminal is **10-14%**; implied growth above 14% against a mature vertical mix is the stress-test flag. Call `get_fair_value_analysis(section='reverse_dcf')` and carry `g`, CoE, and implied-growth explicitly through `calculate`.
