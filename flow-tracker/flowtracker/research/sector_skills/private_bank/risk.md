...
- **Digital-onboarding embargo or supervisory action** — the RBI's April 2024 Kotak Bank embargo on new online/mobile customer acquisition and fresh credit cards (via IT-systems review; lifted February 2025) cost that bank a material share of its retail-acquisition run-rate for several quarters — this category of bank-specific supervisory action is becoming more common as RBI enforces IT-resilience standards.
...
- **FDI Master Direction on Private Banks** — aggregate foreign holding cap at **74% of paid-up** (FPI + FDI + ADR/GDR + NRI combined, with NRI internal sub-limit of 10%). Per-FPI entity-level cap at 10%. Large private banks ran 65-74% aggregate pre-2022, but post-2022 FPI selling has brought them well below the cap (HDFCBANK and ICICIBANK FII alone ~44% each as of 2025/2026) — verify the current aggregate before treating the cap as a binding constraint.
- **RBI macro-prudential risk-weight circular (November 2023, RBI/2023-24/85)** — increased risk-weights on unsecured personal loans from 100% to 125%, on bank credit-card receivables from 125% to 150% (NBFC card receivables 100%→125%), and on bank lending to higher-rated NBFCs by 25 pp. Structural ROE reset for unsecured-heavy private banks; cost 100-200 bps on CRAR for banks running >20% unsecured-retail share.
- **Credit-Deposit (CD) ratio scrutiny** — RBI has flagged elevated system CD ratios; a bank running CD >80-85% with a widening deposit-growth gap faces a soft regulatory constraint on advances growth (RBI dialogue, deposit-mobilisation pressure). Treat a high/rising CD ratio as a cap on sustainable advances growth — the bank must let loan growth decelerate toward deposit growth, which directly caps the `g` carried through valuation models. Cross-check `cd_ratio` against deposit-growth trajectory.
- **RBI Prompt Corrective Action (PCA) framework (revised, effective Jan 2022)** — the three live triggers are Net NPA (>6%), CRAR/CET1 below the regulatory minimum, and the Tier-1 leverage ratio. ROA was **removed** as a PCA trigger in the Jan 2022 revision — do NOT cite "negative ROA triggers PCA". Private banks rarely hit PCA but the framework is live — CRAR below 12.5% with declining trajectory is a PCA-proximity flag.
- **D-SIB buffer requirements** — HDFCBANK and ICICIBANK carry the D-SIB designation requiring an additional CET1 buffer above regular norms, reviewed annually (RBI 2025 list: HDFCBANK 0.40%, ICICIBANK 0.20%; SBI 0.80% for reference). Factor into CRAR-headroom calculation for these two names.
