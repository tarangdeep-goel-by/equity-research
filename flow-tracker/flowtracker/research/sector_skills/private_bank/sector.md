...
- **D-SIB private banks**: HDFCBANK, ICICIBANK. RBI-designated systemically important; per-bank additional CET1 buffer (RBI 2025 D-SIB list: HDFCBANK 0.40% bucket-2, ICICIBANK 0.20% bucket-1; SBI 0.80% bucket-4 for reference); quasi-sovereign premium on funding spreads. Combined market cap 55-65% of the private-bank cohort.
...
- **Aggregate foreign holding vs the 74% cap** — post-2022 FPI selling has pulled the top private banks well below the cap (HDFCBANK and ICICIBANK FII alone ~44% each as of 2025/2026; aggregate foreign far from 74%). The pre-2022 "near-cap" regime no longer holds — verify the current aggregate rather than assuming cap-proximity. Only when aggregate genuinely climbs back >70% does passive-ETF rebalance demand get largely absorbed and incremental foreign flow depend on active-manager decisions.
...
- **ADR / GDR flows** — HDFCBANK and ICICIBANK run ADR programmes on NYSE (HDFCBANK 1 ADR = 3 underlying India shares; ICICIBANK 1 ADR = 2 — use each bank's own ratio). ADR flow dynamics differ from direct FPI — Rule 144A vs Reg-S tranche distinction matters for liquidity analysis.
...
- **UPI / digital disintermediation of fee income** — UPI MDR is zero for all standard bank-to-bank P2M (and P2P) transactions regardless of ticket size, not just sub-₹2000 — so the payment-interchange fee-pool is structurally near-zero across the board. Offsetting, bank wealth-management and loan-origination fee-pool is growing. Net impact on private-bank fee-income share is flat-to-slightly-positive depending on wealth-franchise depth.
...
- **ECL provisioning framework — finalised** — RBI issued final Expected Credit Loss directions on 27 April 2026, effective 1 April 2027 (IFRS-9-aligned staging + product-wise prudential floors; applies to SCBs, excludes RRBs/SFBs/payments banks). Expect a one-time CET1 hit (CRISIL estimates up to ~120 bps, with some managements guiding 100-150 bps), which banks may spread over four years (FY27-FY31). Track management's guided CET1 impact and adjust forward BVPS/CRAR accordingly — do NOT treat ECL as merely "in transition".
- **LCR / NSFR tightening cycles** — RBI periodically recalibrates liquidity-coverage norms. Each recalibration drives bulk-deposit re-pricing across the sector; private banks with strong CASA franchise absorb these cycles better. The revised LCR norms effective 1 April 2026 add a **2.5% additional run-off factor on internet/mobile-banking-enabled retail and small-business deposits** (reduced from the 5% proposed in the July 2024 draft) — digitally-heavy private banks must hold more HQLA against the same deposit base, a structural NIM/LCR headwind. Monitor each bank's LCR trajectory and the digital-deposit share when assessing the drag.
...
- **Cost/Income %** — 38-48% for large private banks; mid-caps in a heavy investment/transition phase run materially higher at 65-75% (e.g. IDFCFIRSTB ~72% in FY25), mean-reverting only over multiple years.
