...
- **ARPU × users revenue-bridge template.** When quantifying revenue trajectory, always show the bridge: `subscribers_mn × arpu_₹_per_month × 12 ÷ 10 = revenue_cr_annualized`. The `÷ 10` is mandatory: subscribers in **millions** × ARPU in **₹/month** × 12 gives revenue in **millions of ₹**, and ₹1 Cr = 10 million ₹, so dividing by 10 converts to crores (e.g. 350 mn subs × ₹200 × 12 = ₹840,000 mn = ₹84,000 Cr). Units: subscribers in millions (or crores — state which), ARPU in ₹/subscriber/**month** (NOT annual) — the 3× annualisation error and the millions-vs-crores unit error are the two most common telecom-financials failure modes. Pass through `calculate(operation='expr', a='<subscribers_mn> * <arpu_monthly> * 12 / 10', b='0')` and cite the output; never hand-math a 9-digit multiplication.
- **Nigeria / international-geography FX devaluation risk.** This applies only to operators that actually have cross-border telecom revenue — principally **Bharti Airtel via its Airtel Africa subsidiary** (14 African markets). Note that **Vodafone Idea operates exclusively in India with no international telecom book**, and **Reliance's Jio-bp is a domestic fuel-retailing JV with no FX-devaluation exposure** — do not impute international-FX risk to India-only operators. For genuinely cross-border operators, local-currency ARPU growth can mask USD/INR-denominated revenue decline after FX devaluation. Cite the constant-currency growth alongside reported growth and identify any >15% cumulative YoY FX devaluation in the relevant geography. Nigeria NGN, Egyptian EGP, Turkish TRY, Argentine ARS have historically surprised investors; the report must state the FX regime in the geography, not just the local-currency operating metric.

### EBITDA One-Offs Check

If a single-quarter spike shows up in EBITDA, cross-check the concall for one-offs (spectrum sale, tower transaction, insurance recovery) before narrating it as operational.
