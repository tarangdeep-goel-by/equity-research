...
- **Peak-cycle dividend-yield trap** — trailing double-digit dividend yield on a mature PSU miner or peak-earnings steel producer is the dividend-equivalent of the peak-PE trap. Payout ratios are typically fixed as a % of PAT, so the absolute dividend collapses linearly with trough earnings; the "yield" is cycle-phase-dependent. Normalize to a through-cycle dividend (payout % × through-cycle EBITDA × (1 − tax) × (1 − reinvestment)) before citing the yield. Cross-check dividend coverage against `get_fundamentals(section='cagr_table')` over 10Y to confirm sustainability through a trough.
