...
- **Capex announced without environmental clearance** — a greenfield/expansion capex number is not a real commissioning timeline until the MoEFCC Environmental Clearance (EC) and the State PCB Consent to Establish/Operate (CTE/CTO) are in hand. EC/CTO grant for Red-Category chemical plants routinely slips 12-24 months and can be denied outright in water-stressed clusters. Verify EC/CTO status before accepting management's commissioning date into the ramp model; an announced-but-uncleared plant is an option, not a scheduled asset.
