...
Indian specialty chemicals and paint sectors are defined by multi-generational family-promoter dominance. Baseline promoter holding is stable at 50-75%. Execute `get_ownership(section='shareholder_detail')` to map cross-holdings via family trusts. The promoter-pledge baseline is sub-type-specific, not sector-wide: specialty-chem and paints sit near zero (<5%), but commodity chemicals routinely carry 5-15% group-level pledge without distress (see Tenet 18 below). For specialty / paints names, any pledge above the <5% baseline is a severe anomaly signaling liquidity stress outside the core chemical business. Verify the baseline against the correct sub-type via `get_ownership(section='promoter_pledge')`.

### Paints Oligopoly Stability & Price Discipline
The Indian paints sector is a concentrated organized oligopoly — the organized segment is ~65-75% of the total market (sources vary; IBEF cites ~65%), historically led by Asian Paints. The oligopoly is now under active disruption: Asian Paints' share has slipped from ~57-59% toward ~50% as Aditya Birla's Birla Opus (~10% share within ~2 years of launch, ~45,000 tinting machines) and JSW Paints (via acquisition) entered with conglomerate-scale capital. So the "stable oligopoly / low-turnover ownership" read is no longer automatic — the historical price discipline (1-2 lead-and-follow price hike rounds annually) is breaking down into a price/discount war. Institutional ownership has historically shown anchor DII / FII stability, but monitor `mf_changes` for structural de-rating or defensive reallocation triggered by these heavily capitalized new entrants.
...
### Mandatory Checklist
- [ ] Verify promoter pledge against the correct sub-type baseline (<5% specialty/paints; 5-15% commodity) via `promoter_pledge`
...
**FII % anchor (specialty):** *"FII stake of X% sits at the Y-th percentile within Indian specialty-chemicals top-tier (PIDILITIND, SRF, AARTIIND, NAVIN FLUORINE, DEEPAKNITR peer set via `get_peer_sector(section='benchmarks')`). Specialty-chem FII benchmarks run higher than commodity-chem because branded-specialty ROCE of 20-30% attracts long-only foreign mandates; commodity chemicals earn cycle-weighted ROCE. But the band is regime-dependent: the FY21-23 China+1 boom pushed top-tier specialty FII toward the high teens / low-twenties, and the FY24-25 de-rate pulled many names back into single digits — e.g., DEEPAKNITR FII is ~6% and AARTIIND ~7% as of 2026 (BSE/NSE shareholding), so a static '18-28% specialty band' is no longer accurate. Always pull the live figure via `get_peer_sector(section='benchmarks')` rather than assuming a fixed band, and read FII direction-of-travel against the China+1 cycle phase."* A 12% FII can be mid-range for commodity-chem and, post-de-rate, near-top-quartile for several specialty names — so the qualitative read depends on both sub-type and cycle phase.
...
**Promoter pledge anchor (chemicals-specific baseline):** Chemicals sector carries a HIGHER baseline promoter pledge than pharma — commodity-chem promoter families routinely pledge up to 8-12% as standing group-level liquidity. This is promoter/group-level financing, not the listed company's working capital: company working capital is secured against current assets (inventory, receivables), while a promoter pledge funds promoter- or holding-company-level debt — so the pledge reflects the family's balance sheet, not the operating cycle. This is UNLIKE the near-zero baseline of pharma/FMCG/IT. Specialty chemicals (PIDILITIND/SRF/AARTIIND/NAVIN/DEEPAKNITR) trend closer to <5%; commodity chemicals (TATACHEM/DCM/GSFC) routinely sit at 5-15% with no distress signal. A pledge at 10% in a specialty name is a red flag; the same 10% in a commodity name is near-baseline. Cite the specialty-vs-commodity classification BEFORE narrating pledge conviction.
