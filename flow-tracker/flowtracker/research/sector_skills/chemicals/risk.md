...
- **Promoter-pledge in mid-cap specialty** — baseline is <5% for specialty per the chemicals ownership archetype. Pledge above 8-10% in a specialty name is a liquidity-stress tell; the same range is near-baseline in commodity chemicals, which routinely sit at 5-15% without distress signal. Note the correct causal mechanism: company working capital is secured against current assets (inventory and receivables), not against promoter shares — promoter pledges fund promoter- or group-level debt, so a pledge tells you about the family's balance sheet, not the listed company's working-capital cycle. Cite the sub-type before narrating pledge severity. Verify via `get_ownership(section='promoter_pledge')`.
...
- **MoEFCC Environmental Clearance (EC) & State PCB Consents (CTE/CTO)** — capex is not commissionable without a MoEFCC EC plus the State PCB Consent to Establish/Operate; for Red-Category chemical plants these routinely slip 12-24 months or are denied in water-stressed clusters, and operating on a lapsed CTO is a critical regulatory/ESG breach. Treat an announced-but-uncleared expansion as a contingent asset, and flag any plant running on an expired consent.
...
- **BIS standards / Quality Control Orders (QCOs)** — the Bureau of Indian Standards sets statutory domestic product-quality norms; QCOs make BIS certification mandatory for specified chemicals and bar non-compliant imports, reshaping domestic pricing power for agrochem and industrial specialty. (CII is an industry trade body and lobby, not a regulator — do not cite it as a standards authority.)
- **DGTR trade remedies — Anti-Dumping Duties (ADD) and their sunset reviews** — India's primary statutory defense against Chinese (and other) dumping. The Directorate General of Trade Remedies (DGTR, Ministry of Commerce) imposes ADD on specific molecules (recent 2026 actions span DASDA, TDI, 2-ethyl hexanol, fluoroelastomer, dye intermediates). ADD is time-limited (typically 5 years) and lapses unless renewed via a **sunset review** — so the single most overlooked margin risk for an import-competing domestic producer is an ADD expiring without renewal, which re-opens the market to dumped imports and compresses realization. Track DGTR initiation/final-findings notifications and sunset-review timelines for the company's key molecules; an active ADD is a pricing-power tailwind whose expiry must be modeled into normalized margins. Conversely, a domestic player dependent on imported KSM can be hurt when ADD is imposed upstream.
