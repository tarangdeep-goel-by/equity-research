...
| **Tobacco / alcobev** | PE + dividend yield (EV/EBITDA is valid for tobacco — EBITDA is already net of excise under Ind AS) | 18-28× PE, 3-5% yield | P/B; cross-period EV/EBITDA comparisons across an excise/GST-rate change |
...
### Justified Multiple — Carry `g` and ROCE Explicitly
FMCG's structurally high multiples are mathematically tethered to high ROCE, durable volume-CAGR (`g`), and low cost-of-equity. The re-rating / de-rating lever is these three inputs, not a narrative. Use the Gordon-growth PE approximation for a sanity check whenever a multiple is being defended:

`Justified PE ≈ (1 − g / ROCE) / (CoE − g)` for a steady-state FMCG franchise, where `g` = sustainable earnings-growth CAGR (volume × realization, typically 8-14% for HPC leaders, 6-10% for food, 4-8% for staples, 6-10% for tobacco), `ROCE` = post-tax return on capital employed (commonly 40-80% for HPC leaders, 20-35% for food & staples, 25-45% for tobacco), and `CoE` = cost of equity (current Indian regime ~11-13% for large-cap FMCG).

A 45-60× PE for an HPC leader is justified when ROCE ≥ 50% and `g` ≥ 10%; if ROCE compresses to 30% (margin pressure) or `g` decays to 5% (volume stagnation), the justified multiple compresses to 25-35×. The "double-whammy" de-rating common in mid-cap FMCG names trading at 40×+ PE: volume growth stalling to low-single digits both drops EPS (numerator) AND compresses the justified multiple (denominator), producing 40-55% drawdowns in aggregate. Route the arithmetic through `calculate` with `roce_pct`, `g_pct`, `coe_pct` as named inputs; never defend a multiple without naming all three.
...
### Volume-vs-Price Decomposition & A&P Intensity (mandatory for FMCG)
Two FMCG-specific drivers must appear in the valuation — both feed the `g` and moat inputs above:
- **Volume vs price/mix split of revenue growth.** Decompose reported revenue growth into underlying volume growth vs price/mix (realisation), from `get_company_context(section='concall_insights', sub_section='operational_metrics')` / `management_commentary`. Volume-led growth supports a higher sustainable `g` and multiple; price/mix-led growth in a soft-volume environment is lower-quality and caps the multiple. If management has not disclosed the split, state that — do not infer a split you cannot source.
- **A&P (advertising + promotion) as % of sales** and its YoY trend, from the AR other-expense schedule (`get_company_context(section='annual_report', sub_section='notes_to_financials')`) or concall. A&P is the brand-moat reinvestment gauge: a multiple defended on "brand strength" while A&P-intensity is falling is leaning on harvesting, not moat. A&P is rarely a separate line in structured `expense_breakdown` — extract from the notes; if genuinely undisclosed, state the gap.

### Historical PE Band Context — FMCG Premium to Nifty
A 5-10Y PE band via `get_chart_data(chart_type='pe')` gives the long-arc context. FMCG leaders structurally trade at a very large premium to the Nifty PE — typically 100-150%+, not the modest 25-40% that applies to the broad consumer-staples basket. With the Nifty at ~20-22×, HUL trades ~45-55×, Nestle India ~70-80×, and the FMCG industry average ~48×; the premium is justified by earnings visibility, negative-working-capital economics, and defensive-cash-flow profile but is structurally far above index. The common eval error is citing "trading at 50× PE, expensive vs Nifty at 22×" without the sector-premium context — 50× against a 10Y FMCG-leader median of ~52× is closer to fair than stretched. Always report: current PE vs 5Y/10Y sector median vs 5Y/10Y own-stock median. Anchor the band via `get_valuation(section='band', metric='pe')` with `get_chart_data(chart_type='pe')` as the deep-history fallback when the band call returns fewer than 20 quarterly observations.
...
4. Apply a 10-20% **conglomerate discount** to the aggregate value before comparing to the current market cap (this is the correct term for an *operating* multi-business company like ITC). Reserve the larger 45-70% **holding-company / NAV discount** for pure investment vehicles that only hold stakes in other listed companies — do not apply that magnitude to an operating conglomerate.
...
- **EV/EBITDA on tobacco** — this is *not* actually broken: under Ind AS 115 excise duty is expensed above EBITDA (it is netted out before EBITDA, unlike GST which is excluded from revenue), so reported EBITDA is already net of excise and EV/EBITDA is mathematically sound — brokerages routinely value ITC's cigarette business on EV/EBITDA (e.g. cut to ~11x post the Feb-2026 tax hike). PE + dividend yield remain useful complements for sin-goods, but do not dismiss EV/EBITDA as excise-distorted. The one genuine caveat is comparability across periods when the excise/GST rate itself changes (FY26 cigarette revenue is not comparable to prior years). Alcobev is the genuinely awkward case — net revenue (post state-excise) is the right base, so confirm the revenue convention before applying any multiple.
