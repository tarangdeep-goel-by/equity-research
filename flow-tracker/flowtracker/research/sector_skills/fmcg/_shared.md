...
- **Segment P&L for multi-segment FMCGs.** For companies running ≥2 reportable segments — Home Care / Beauty & Personal Care / Foods & Refreshment (HUL, Marico, Dabur), Paints / Chemicals (Asian Paints, Berger), or diversified conglomerates (ITC: cigarettes / FMCG-others / hotels / paperboards / agri) — extract and state segment revenue, segment EBIT, and the margin gap between segments from concall `financial_metrics`. Note that several single-category foods players (Britannia, Nestle India) report just ONE statutory segment ("Foods") because the Chief Operating Decision Maker reviews the business at company level — for these, statutory segment-P&L is unavailable and you must fall back to management-disclosed category splits (biscuits vs cakes vs dairy; milk-products vs prepared-dishes vs beverages vs confectionery) from the concall/deck, labelling them as voluntary disclosure, not audited segment data. Consolidated margin is a blended number that hides which category is the margin driver. For Unilever/ITC-style multi-segment FMCGs, segment-level analysis is NOT optional.
