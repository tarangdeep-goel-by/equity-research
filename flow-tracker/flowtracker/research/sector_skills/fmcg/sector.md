...
- **Tax regime on sin-goods** — keep the two regimes separate: **tobacco / aerated drinks** sit inside GST (now the 40% sin-goods slab under the Sep-2025 GST 2.0 recast, plus Central Excise on tobacco after the compensation cess wound down on 1 Feb 2026) and are repriced by the GST Council nationally; **alcohol for human consumption is constitutionally excluded from GST** and is governed purely by state excise, so its trajectory is set state-budget-by-state-budget. Each state budget can reprice alcobev economics materially; GST Council action reprices tobacco/aerated economics.
...
- **Rural cycle (2-3Y mini-cycle)** — drivers are monsoon, agri-income, MNREGA wages, rural-infra spend. Current phase (FY26): rural has *led* the recovery, outpacing urban volume growth for ~6-8 consecutive quarters (rural ~7-8% vs urban ~3-5% through 2025, gap narrowing into late-FY26). Re-verify the latest NielsenIQ print rather than assuming rural is the laggard — the prior "recovering from a slowdown" framing is stale.
...
- **Top-3 food & beverage leaders** — biscuits, dairy, beverages, chocolates — tier-1 names dominate category share with cold-chain + plant footprint. HORECA (hotels / restaurants / café / out-of-home) is 15-25% of beverage and packaged-food volume; OOH growth tracks discretionary-spend recovery and is cyclically distinct from GT retail, re-accelerating post-COVID.
...
- **Private-label penetration in MT and QC** — tier-1 MT chains reach 15-20% private-label share in select categories (detergents, packaged staples); QC platforms are the newer and more acute threat — they control the captive-UI search rank and can scale their own private-label SKUs in staples, snacks, and beverages faster than legacy MT chains. Creates gross-margin pressure on mid-tier brands first; leaders with moat in brand-equity and ad-spend are selectively insulated but not immune.
