...
### Standalone Health Insurer — P/B-ROE with Combined-Ratio Anchoring
Standalone-health names (STARHEALTH, NIVABUPA) need a dedicated lens — general-insurer P/B is not interchangeable because the retail-individual claims mix is structurally different. **Primary multiple: P/B-ROE** (Gordon) with combined-ratio anchoring — justify the ROE via loss-ratio + expense-ratio decomposition rather than inheriting the general-insurer band. A combined ratio marginally above 100% (top listed names ~100-103% in FY26, guiding toward sub-100%) reflects retail claims skew, so underwriting is typically marginally loss-making; ROE comes from float plus mix shift, and the **growth engine is GWP growth 18-25%**, not float accretion.

### Reinsurer — P-to-EV on Life Leg and P/B-ROE on P&C Leg
The Indian reinsurance universe is narrow — **GIC Re is the sole Indian reinsurer** (listed, state-backed, receives obligatory cessions); foreign reinsurer branches (since the 2016 amendments) run smaller books. (Do not treat "New India" as a reinsurer — New India Assurance is a primary general insurer, not a reinsurer.) The multiple depends on which cedant book dominates. Split the valuation lens by leg: life-reinsurance treaty books trade on **P-to-Embedded-Value** (similar to primary life insurers but with a lower justified P/EV reflecting cedant-credit pass-through and the cat-retention volatility overlay); property-casualty treaty books trade on **P/B-ROE** normalised across the cat cycle — a single-year PE spike in a cat-loss year is mean-reverting pessimism, not a derating. Retrocession structure (the share of ceded treaty that the reinsurer itself lays off) and catastrophe-retention limits drive underwriting-result volatility; a rising retention ratio into a cat cycle is a ROE-vulnerability tell. Solvency floor is 150% per IRDAI, identical to primary insurers; buffer drift toward 160-170% without a capital action is the supervisory-attention signal.
...
### LICI — Sovereign-Backed, Par-Dominated, Not Peer-Comparable on P/EV
LICI (Life Insurance Corporation of India) is not directly peer-comparable to private-sector listed life insurers and cannot be valued off the private-insurer P/EV band (1.8-3.2×) without explicit adjustments. Three LIC-specific structural factors reshape the multiple:
- **Sovereign backing under Section 37 of the LIC Act** — policyholder sum-assured is guaranteed by the Government of India. This is a unique credit attribute that no private insurer carries; it lowers LIC's cost-of-capital anchor for the policyholder franchise but does not automatically accrue to shareholder P/EV (it accrues to the policy, not the equity claim). Do not double-count sovereign backing as a shareholder P/EV premium; it is a policyholder protection, not a shareholder one.
- **Par-heavy book → structurally lower VNB margin** — LICI's new business remains par-concentrated, producing a VNB margin below the 25-30% of top-tier private insurers with diversified non-par + protection + ULIP mix. The gap is narrowing as non-par share rises — LIC's VNB margin reached ~21.2% in FY26 (vs ~17.6% FY25), so use the current disclosed margin rather than a stale 13-17% band, but still do not benchmark LICI's VNB margin straight against the private-insurer median; the mix is apples-to-oranges.
- **Shareholder-surplus distribution regime** — LICI operates a policyholder/shareholder surplus split regime that has been revised post-listing (historic 5% shareholder share of par surplus shifting toward a higher share over a staged timetable). This is a structural shareholder-economics shift that gradually lifts reported RoEV without new-business change; carry the staged transition as an explicit assumption and check `get_company_context(section='concall_insights', sub_section='management_commentary')` and `get_events_actions(section='material_events')` for the current-quarter regime disclosure. Do not smooth across the transition.

Anchor LICI's justified P/EV at a structurally lower range than private peers to reflect the par-mix and the not-yet-fully-transitioned surplus regime; explicitly state the LIC-specific multiple in the narrative rather than flagging a "discount to private peers" as if it were a mispricing.
