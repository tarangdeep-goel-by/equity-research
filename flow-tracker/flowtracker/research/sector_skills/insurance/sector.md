...
- **Life insurers — top-3 by APE plus LICI** — LICI is a structural market-share shifter (private sector gains ~100-200 bps share annually against LIC over the last decade; the pace of shift is a watch variable). Top-3 private: SBILIFE, HDFCLIFE, ICICIPRULI. Mid-tier: MAXFIN (Max Life via holdco Max Financial), BAJAJFINSV (Bajaj Allianz Life via Bajaj Finserv). **LICI is not peer-comparable to private insurers on VNB margin or P/EV** — a still-par-heavy book produces a structurally lower VNB margin than top private peers (25-30%), though LIC's margin has improved materially as non-par share rises — ~21.2% in FY26 (up from ~17.6% FY25) on a 35% non-par share of individual APE, Section 37 of the LIC Act provides a sovereign guarantee on sum assured (a policyholder protection, not a shareholder P/EV premium), and the post-listing par-surplus shareholder-distribution regime is on a staged transition that lifts reported RoEV mechanically. Benchmark LICI only against itself across the regime transition, not against private peers.
...
- **Rising penetration of health + protection** — health-insurance premium penetration is <1% of GDP (non-life total ~0.9-1% of GDP; health is the largest non-life line at ~41% of non-life GDPI) and term-protection penetration is also low — both far below developed-market levels (do not confuse insurance penetration with total healthcare expenditure, which runs ~3-4% of GDP). This is a multi-decade tailwind; the 22 Sept 2025 GST exemption on individual life/health and any sum-insured mandate accelerate adoption.
...
- **Bima Sugam — IRDAI's insurance-distribution marketplace (phased rollout from Dec 2025)** — after repeated delays the regulator-sponsored centralised policy-purchase and servicing portal (built by the Bima Sugam India Federation) began a phased Phase-1 rollout in December 2025, starting with motor (life and health segments delayed on integration). It is a 3-5Y distribution-economics reset: for life insurers it formalises a mis-selling crackdown via centralised-transparent distribution (compression of commission rents on par/non-par savings products), and for insurtech marketplaces it is a flow-through on retail discovery (the platform hosts rather than disintermediates broker / aggregator contributions). Watch the segment-by-segment go-live timetable and the commission-structure notification.
- **GST exemption on individual policies (effective 22 Sept 2025)** — the 56th GST Council cut GST to 0% on all individual life and individual health policies (group/employer policies stay at 18%). Net effect is positive for affordability/volume on the individual book, but insurers lose input-tax-credit on the exempted lines (a partial cost/margin offset) — model the affordability uplift and the ITC drag separately rather than treating it as a pure tailwind.
- **100% FDI + composite licences (Insurance Laws (Amendment) Act, 2025)** — the FDI ceiling rose to 100% (automatic route), opening M&A / promoter-buyout activity; composite licences now let a single insurer write both life and general/health, which structurally threatens the standalone-health moat (STARHEALTH, NIVABUPA) as large life insurers can enter health. Screen for M&A targets post-liberalisation and for pricing/share pressure on standalone health.
