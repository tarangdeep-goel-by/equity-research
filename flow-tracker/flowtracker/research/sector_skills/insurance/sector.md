...
- **Life insurers — top-3 by APE plus LICI** — LICI is a structural market-share shifter (private sector gains ~100-200 bps share annually against LIC over the last decade; the pace of shift is a watch variable). Top-3 private: SBILIFE, HDFCLIFE, ICICIPRULI. Mid-tier: MAXFIN (Max Life via holdco Max Financial), BAJAJFINSV (Bajaj Allianz Life via Bajaj Finserv). **LICI is not peer-comparable to private insurers on VNB margin or P/EV** — par-dominated book produces 13-17% VNB margin (vs 25-30% private), Section 37 of the LIC Act provides a sovereign guarantee on sum assured (a policyholder protection, not a shareholder P/EV premium), and the post-listing par-surplus shareholder-distribution regime is on a staged transition that lifts reported RoEV mechanically. Benchmark LICI only against itself across the regime transition, not against private peers.
