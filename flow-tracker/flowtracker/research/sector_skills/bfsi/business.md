...
| **Life insurer** | VNB = APE × VNB margin; float × investment yield | float + persistency | VNB per policy, 13/61-month persistency |
| **General insurer** | Underwriting profit + float income | combined ratio + float | combined ratio, loss ratio by line |
...
| **Exchange / broker** | Take-rate × ADTV (cash + F&O) | volume × take-rate | active clients, ADTV per client |

### Revenue Decomposition — Always (A × B), Never a Single Line
For lenders, `NII = NIM × Average Earning Assets`; growth decomposes into volume (advances growth), price (NIM shift), and mix (retail/corporate re-pricing at different yields). For AMCs, `Fee revenue = AUM × blended fee yield bps`; AUM growth alone is misleading if the mix shift is toward passive/debt (lower bps). For brokers and exchanges, `Revenue = ADTV × take-rate × days`; the 2023-24 rise of zero-brokerage models collapsed cash-segment take-rates and forced reliance on F&O contract notes. For insurers, `Premium = number of policies × ticket size × persistency`; VNB layering sits on top. Call `get_fundamentals(section='revenue_segments')` and `get_company_context(section='concall_insights', sub_section='operational_metrics')` for the decomposition.
...
- **Liability franchise (banks)** — CASA ratio 40-55% in top private banks; 30-42% in large PSUs; <25% for wholesale-funded NBFCs. Cheap deposits compound through rate cycles. Source via `get_quality_scores(section='bfsi')` and `get_company_context(section='concall_insights', sub_section='management_commentary')`.
...
- **Combined ratio sustained >100%** for general insurers signals the underwriting book is subsidised by float income — a structurally fragile model when yields compress.
...
- "Is the combined ratio sub-105% sustainable if investment yield compresses 50-80 bps on a 10Y G-sec rally?" (general insurers)
