...
- **RBI Prompt Corrective Action (PCA) proximity** — the 2021 revised PCA framework (effective Jan 2022) monitors three parameters only: **capital** (CRAR / CET1 below the CCB-inclusive minimum), **asset quality** (net NPA, e.g. >6%), and **leverage** (Tier-1 leverage ratio). ROA / profitability was **removed** as a trigger — do not cite negative ROA as a PCA trigger. The watch-list status alone reprices the stock 10-20%.
...
- **RBI** — **Basel III norms** (CRAR, CET1, LCR, NSFR minima), **PCA framework** (capital, asset-quality, leverage triggers — ROA was dropped in the 2021 revision), **Large Exposure Framework** (single counterparty 20%, group 25% of Tier-1), **Priority Sector Lending** targets (40% of ANBC, sub-targets for agri and weaker sections; PSL shortfalls force RIDF deposits at low yield, a structural NIM drag), **elevated risk weights on unsecured consumer credit** (Nov 2023: 125% on unsecured personal loans, 150% on credit-card receivables for banks / 125% for NBFCs — these cap growth in the highest-NIM segments and depress CRAR), **asset classification and provisioning** master direction, **digital-lending guidelines** (for NBFCs operating via LSP models), and the finalized **ECL provisioning directions** (issued Apr 2026, effective Apr 2027 — one-time CET1 hit up to ~120 bps, spreadable over FY27-FY31).
...
- **100 bps repo-rate move** — NIM impact 10-25 bps for retail-heavy banks with a high share of repo-linked **EBLR** (External Benchmark Lending Rate) loans (MCLR is an internal benchmark and reprices slowly with a lag, not directly with repo); AFS book MTM on a 10Y-duration bond book is roughly 7-8% for a 100 bps parallel shift, which flows through P&L for held-for-trading and reserves for HTM/AFS.
