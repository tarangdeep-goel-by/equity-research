...
Adjusted Book Value strips the portion of book represented by net non-performing assets that have not yet been fully provisioned. The computation is: `ABV per share = BVPS − (Net NPA ÷ shares outstanding)`. Current `P/ABV = CMP ÷ ABV per share`. Compare against the 5Y P/ABV band via `get_valuation(section='band', metric='pb')` with `get_chart_data(chart_type='pbv')` as the deep-history fallback when the band call returns fewer than 20 quarterly observations. The skip-step surfaced in prior valuation evals is computing P/B against reported BVPS and forgetting the Net NPA deduction. The deduction matters most when NNPA is elevated; note that aggregate PSU-bank net NPA has fallen to a record low (~0.4% as of FY26), so for clean PSU banks the deduction is now small (well under 1% of BVPS) — but it is still load-bearing for any stressed or mid-tier lender carrying NNPA of 1-3%+, where the deduction can be ~3-5% of BVPS and materially changes the re-rating narrative. Always compute it from the bank's actual current NNPA, not a stale sector range. Route the arithmetic through `calculate` with `shares_out`, `bvps`, and `net_npa_per_share` as named inputs.

### Deposit-Franchise Metrics Belong in the Narrative
Surface the **Credit-Deposit (CD) ratio** (`get_sector_kpis` → `cd_ratio_pct`) and **CASA ratio** (`casa_ratio_pct`) in the valuation narrative, not only in the JSON briefing. In the current RBI-tightening cycle the CD ratio is a primary constraint on loan growth — an elevated or rising CD ratio (private banks pushing 90%+) caps the sustainable growth `g` that feeds the justified P/B, while a high-CASA franchise lowers cost-of-funds and supports a premium. A re-rating thesis that ignores the deposit-side constraint is incomplete. Two capital headwinds also feed `g` and sustainable ROE: (1) RBI's Nov-2023 **elevated risk weights** on unsecured personal loans (125%) and credit cards (150% for banks) raise RWA-density and depress CRAR in the highest-NIM segments — factor this when projecting capital-constrained growth; (2) the **ECL transition** (effective Apr 2027) carries a one-time CET1 hit of up to ~120 bps and structurally resets forward ABV — adjust forward book and CRAR for the bank's disclosed transition estimate rather than projecting off pre-ECL book.
...
3. For each **unlisted** subsidiary, apply a sector multiple: AMC at **3-5% of AUM** (premium names toward the higher end), life insurer at **1.5-3× embedded value** (growth × VNB-margin premium), general insurer on **P/B (or a multiple of GWP / GDPI)** — note "annual premium income" is a life-insurance concept and does not apply to general insurers — NBFC arm at **1.0-2.5× book** (calibrate to subsidiary-specific ROE and credit profile).
...
- "(Insurers) Has the company begun Ind AS 117 (IFRS 17) parallel reporting? IRDAI mandates phased adoption from ~FY27, shifting profit recognition to a Contractual Service Margin (CSM) released over the policy term rather than upfront — check the disclosed transition impact on net worth and reported earnings, and keep P/EV (life) as the anchor since accounting-PE optics will be in flux through the transition."
