...
- **NIM cycle** — split the advances book by benchmark: EBLR-linked (external benchmark, usually repo-linked) re-prices near-instantly (0-1 quarter lag) in either direction, while MCLR-linked re-prices with a 2-3 quarter lag. For banks with a high share of EBLR-linked retail advances (typically 50-70% of the floating-rate book post-2019 mandate), a falling-rate cycle compresses NIM fast — loan yields drop before term-deposit re-pricing catches up. The old "loans lag deposits" framing only holds for MCLR-heavy corporate books; EBLR-heavy retail books show the reverse lag. State the EBLR / MCLR split from the management's asset-book disclosure before projecting the NIM direction.
...
- **D-SIB-designated banks** — RBI currently designates exactly **three** Domestic Systemically Important Banks: **SBI** (bucket 4), **HDFC Bank** (bucket 2), **ICICI Bank** (bucket 1). These carry an additional CET1 surcharge, are the balance-sheets RBI backstops, and command the systemic-premium in valuation. (The next tier of large banks by advances/deposits — e.g. Axis, Kotak, PNB, BoB — are systemically large but NOT D-SIB-designated; do not call them D-SIBs.)
...
- **NBFC tiers** — by product: mortgage (HFC), vehicle finance, consumer, MFI, gold-loan, wholesale; each tier has distinct cost-of-funds, loss-given-default, and rate-cycle sensitivity profiles. Also anchor each NBFC to its **RBI Scale-Based Regulation (SBR) layer** — Base (small, non-deposit, <₹1,000 Cr assets), Middle (all deposit-taking NBFCs + non-deposit ≥₹1,000 Cr), Upper (RBI-identified systemically-important NBFCs facing bank-like norms — CET1/leverage, mandatory listing, tighter NPA recognition) and Top (extreme-risk, normally empty). The SBR layer dictates capital, NPA-recognition, and compliance burden, so it materially differentiates two NBFCs in the same product tier.
...
- **ECL transition** — the IndAS 109 / ECL (Expected Credit Loss) accounting framework changes the timing of provisioning and the reported ROE trajectory. RBI issued **final ECL directions on 27 April 2026, effective from 1 April 2027** (commercial banks ex-SFB/PB/LAB). It carries a one-time CET1 hit of up to ~120 bps system-wide (higher for PSBs / mid-tier banks, networth impact 3-9%; large private banks better cushioned by 2-4% provision buffers), spreadable over a four-year glide-path (FY27-FY31). Extract management commentary on each bank's estimated transition impact and adjust forward ABV / CRAR accordingly; this is a structural reset, not a future "in progress" item.
