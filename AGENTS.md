...
**Never commit autoeval artifacts.** `flow-tracker/flowtracker/research/autoeval/eval_history/`, `run_logs/`, `last_run.json`, and `results.tsv` are all generated by grader runs and are `.gitignore`d. If you see them appear in a diff, stop and drop them from git tracking.
