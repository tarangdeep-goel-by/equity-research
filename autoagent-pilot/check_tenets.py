#!/usr/bin/env python3
"""Verify no load-bearing preamble rule or business Key Rule was deleted.

Exit 0 if the current `prompts.py` preserves every L1 section header and
every L2 business Key Rule bullet that existed in baseline/prompts.py.
Exit 1 (with a diff) otherwise.

Meta-agent runs this BEFORE every commit of an L1 or L2 edit. If it exits
non-zero the edit is abandoned via `git reset --hard HEAD` and the next
iteration tries a different shape of fix.
"""
...
def l2_key_rules(business_system: str) -> list[str]:
    """Business L2 Key Rules — bullets inside the '## Key Rules' section."""
    m = re.search(r"^## Key Rules\s*\n(.*?)(?=^## |\Z)", business_system, re.DOTALL | re.MULTILINE)
    if not m:
        return []
    block = m.group(1)
...
def main() -> int:
    ...
    # ---- L2 business Key Rules check --------------------------------------
    b_l2 = l2_key_rules(extract_block(baseline_src, "BUSINESS_SYSTEM_V2"))
    c_l2 = l2_key_rules(extract_block(current_src, "BUSINESS_SYSTEM_V2"))
    ...
    if deleted_l2:
        ok = False
        print("TENET GUARD VIOLATION — L2 BUSINESS_SYSTEM_V2 Key Rules deleted:")
        for r in deleted_l2:
            print(f"  - {r}")
    if deleted_tenets:
        ok = False
    ...
    if not ok:
        print()
        print("Rule: load-bearing preamble sections and business Key Rules may be")
        ...
        print()
        print("Revert the edit: git reset --hard HEAD")
        return 1

    print(f"OK — L1 sections: {len(c_l1)}/{len(b_l1)} preserved, "
          f"L2 business Key Rules: {len(c_l2)}/{len(b_l2)} preserved, "
