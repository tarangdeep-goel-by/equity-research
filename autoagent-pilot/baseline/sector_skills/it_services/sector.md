...
- **AI / GenAI productivity** — tools like Copilot, Cursor, internal agent platforms reduce billable hours on legacy T&M work 20-40% on pilots; enterprise-scale re-pricing not yet in the reported P&L, but the lead indicator is fixed-fee-deal share rising in the book-to-bill mix. The countervailing "plumbers of AI" thesis is that Indian tier-1 services are uniquely positioned to monetise enterprise AI-adoption: change-management, legacy-integration, data-migration, RAG-pipeline build-out, and model fine-tuning / MLOps work is labour-intensive services revenue that scales with client AI deployment. Net effect on the services book is ambiguous — T&M compression on legacy AMS vs new AI-implementation revenue streams. Diagnose which side dominates by tracking (a) AI-practice revenue disclosure as a share of total, (b) fixed-fee-deal-share momentum, (c) revenue-per-FTE trajectory (rising = AI-adoption benefit, falling = disruption winning).
...
- **GCC threat in ER&D — vertical-specific** — GCCs in aerospace (Boeing, Airbus, Safran, Honeywell), automotive (GM, Ford, Stellantis, BMW, Mercedes), semiconductor (Qualcomm, AMD, Intel, Micron, Nvidia) and BFSI-platforms (JPMorgan, Goldman, Wells, Citi) are building 2-5k engineer captives in India that compete head-on with ER&D-specialist vendors on the same domain depth. The ER&D sub-type (archetypes include LTTS / TATATECH / CYIENT / KPIT-style vendors) is more exposed than services because engineering-domain IP is narrower and more easily captured in a captive model than broad services scale. Track GCC-engineer-count by vertical and listed-ER&D-vendor top-client share in the same vertical for a read on competitive erosion.
